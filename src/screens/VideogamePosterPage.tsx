...
export type VideogamePosterPageProps =
  | { title: string; mode: 'play'; status: 'backlog' | 'played' }
...
export default function VideogamePosterPage(props: Props) {
  ...
  const load = useCallback(async () => {
    ...
    if (mode === 'nine_club') {
      ...
      orderCol = status === 'played' ? 'played_at' : 'added_at';
    }
  ...
  const moveToBacklog = async (item: VideogameItem) => {
    await supabase
      .from('videogame_items')
      .update({ status: 'backlog', played_at: null })
      .eq('id', item.id);
    setActionItem(null);
    load();
  };
  ...
  const removeFromNineClub = async (item: VideogameItem) => {
    await supabase
      .from('videogame_items')
      .update({ nine_club_added_at: null })
      .eq('id', item.id);
    setActionItem(null);
    load();
  };
